...
function safeGetItem(key, defaultVal = null) { try { return localStorage.getItem(key) || defaultVal; } catch(e) { return defaultVal; } }
...
function checkAndReset3AM() {
    ...
    if (vnTime.getHours() < 3) resetLimit.setDate(resetLimit.getDate() - 1);
    ...
    if (lastResetMark !== currentResetMark) {
        safeSetItem('opened_today', 0); safeSetItem('last_reset_3am', currentResetMark); return 0;
    }
...
function recordBoxOpen(currentEndTime) {
    ...
    if (lastCounted !== currentEndTime.toString()) {
        openedToday++; safeSetItem('opened_today', openedToday); safeSetItem('last_counted_endtime', currentEndTime.toString());
    }
}
...
if (!deviceId) {
    ...
    deviceId = `VIP-${Array.from({length:4},()=>chars[Math.floor(Math.random()*chars.length)]).join('')}-${Array.from({length:4},()=>chars[Math.floor(Math.random()*chars.length)]).join('')}`;
    safeSetItem('vip_device_id', deviceId);
}
if (displayDeviceId) displayDeviceId.textContent = deviceId; 
if (btnCopyDeviceId) btnCopyDeviceId.addEventListener('click', () => { navigator.clipboard.writeText(deviceId); btnCopyDeviceId.textContent = '✅ ĐÃ COPY MÃ'; setTimeout(() => { btnCopyDeviceId.textContent = '📋 COPY MÃ GỬI ADMIN'; }, 2000); });

// ================= CƠ CHẾ ROLE (NHÂN SỰ / VIP) =================
let isFreeMode = false; let deviceStatus = 'unknown'; let adminOffset = 0; 
let isFreeModeLoaded = false; let isDeviceLoaded = false; let currentPingMs = 0; 
let myRole = 'vip'; 
let staffSettings = { ping_interval: 3000, network_mode: 'free', link_free: '' };
let currentPingInterval = 1000;
let currentActiveWsUrl = '';

const deviceRef = ref(db, `devices/${deviceId}`);
const lastAccessState = safeGetItem('last_access_state');
if (lastAccessState === 'granted' && lockScreen) lockScreen.style.display = 'none'; else if (lockScreen) lockScreen.style.display = 'flex';

function pushHeartbeat() {
    if (document.hidden) return; 
    update(deviceRef, {
        last_active: Date.now(), opened_today: openedToday, 
        current_task: { rate: paramM || 'Đang chờ...', coins: coins || 0, can_open: canOpen || 0, end_time: endTime || 0, ping: currentPingMs, flag: hotBoxStr, client_offset: timeOffset }
    }).catch(()=>{});
}
...
function checkAccess() {
    ...
    if (deviceStatus === 'locked') {
        if (lockScreen) lockScreen.style.display = 'flex'; safeSetItem('last_access_state', 'denied'); return;
    }
    if (isFreeMode || deviceStatus === 'active') {
        if (lockScreen) lockScreen.style.display = 'none'; safeSetItem('last_access_state', 'granted'); pushHeartbeat(); 
    } else {
        if (lockScreen) lockScreen.style.display = 'flex'; safeSetItem('last_access_state', 'denied');
    }
}
...
function applyNetworkSettings() {
    ...
    if (myRole === 'staff') {
        targetInterval = staffSettings.ping_interval || 3000;
        if (staffSettings.network_mode === 'free' && staffSettings.link_free) {
            targetUrl = staffSettings.link_free;
        }
    }

    // NẮN LẠI NHỊP PING NẾU ADMIN CHỈNH
    if (currentPingInterval !== targetInterval) {
        currentPingInterval = targetInterval;
        startWsPing(); 
    }

    // SẬP CẦU DAO WSS NẾU ADMIN ĐỔI MẠNG (NGẮT ĐỂ NÓ TỰ CONNECT LẠI)
    if (targetUrl && currentActiveWsUrl && currentActiveWsUrl !== targetUrl) {
...
onValue(ref(db, 'global_settings/free_mode'), (snapshot) => { isFreeMode = !!snapshot.val(); isFreeModeLoaded = true; checkAccess(); });

onValue(ref(db, 'global_settings/staff_settings'), (snapshot) => {
    const data = snapshot.val();
    if(data) {
        staffSettings = { ...staffSettings, ...data };
        applyNetworkSettings();
    }
});

onValue(deviceRef, (snapshot) => {
    isDeviceLoaded = true; const data = snapshot.val();
    if (data) {
        ...
        if (deviceStatus === 'active' && data.note) {
            if (nameInputEl) {
                if (nameInputEl.value !== data.note) { nameInputEl.value = data.note; safeSetItem('myName', data.note); pushNameToFirebase(data.note); }
                nameInputEl.disabled = true;
            }
        } else { if (nameInputEl) nameInputEl.disabled = false; }
    } else { deviceStatus = 'unknown'; adminOffset = 0; if (nameInputEl) nameInputEl.disabled = false; }
    forceUpdateClock(); checkAccess();
});

setInterval(pushHeartbeat, 30000);
onDisconnect(deviceRef).update({ last_active: Date.now(), 'current_task/rate': 'Đã thoát Web...', 'current_task/end_time': 0 });

document.addEventListener("visibilitychange", function() {
    if (document.hidden) { 
        if (myViewerRef) remove(myViewerRef); 
        update(deviceRef, { 'current_task/rate': 'Đang ẩn Tab...', 'current_task/end_time': 0 }).catch(()=>{});
    } else {
...
if (btnSwitchUI) btnSwitchUI.addEventListener('click', () => { const newUI = safeGetItem('active_ui', 'T1') === 'T1' ? 'T3' : 'T1'; safeSetItem('active_ui', newUI); window.location.href = (newUI === 'T3' ? 't3.html' : 'index.html') + window.location.search; });

let mySessionId = safeGetItem('mySessionId');
if (!mySessionId) { mySessionId = 'sess_' + Date.now() + '_' + Math.floor(Math.random() * 10000); safeSetItem('mySessionId', mySessionId); }
...
function setupRoomViewers(newEndTime) {
    ...
    unsubscribeViewers = onValue(roomViewersRef, (snapshot) => {
        const data = snapshot.val();
        if (data) {
            const uniqueNames = [...new Set(Object.values(data).filter(n => n.trim() !== ''))];
            if(viewersList) viewersList.innerHTML = uniqueNames.length > 0 ? `<b>${uniqueNames.join(', ')}</b> đang cày chung` : `Chưa có ai điểm danh`;
        } else { if(viewersList) viewersList.innerHTML = `Chưa có ai điểm danh`; }
    });
}

function pushNameToFirebase(name) { if (myViewerRef) { if (name && name.trim() !== '') set(myViewerRef, name.trim().toUpperCase()); else remove(myViewerRef); } }
if (nameInputEl) nameInputEl.addEventListener('input', function() { safeSetItem('myName', this.value); pushNameToFirebase(this.value); });
setupRoomViewers(endTime);
...
const customWsUrl = urlParams.get('ws') || safeGetItem('ws_url');
...
function connectWebSocket() {
    let connectUrl = customWsUrl;
    
    // ÉP LINK FREE NẾU LÀ NHÂN SỰ VÀ CẦU DAO ĐANG GẠT SANG FREE
    if (myRole === 'staff' && staffSettings.network_mode === 'free' && staffSettings.link_free) {
        connectUrl = staffSettings.link_free;
    }

    if (!connectUrl) return; 
    currentActiveWsUrl = connectUrl;
    ...
    ws.onmessage = (event) => {
        try {
            ...
            if (data.action === 'pong') {
                const now = getAccurateTime(); const rtt = now - data.client_time; const pingMs = rtt / 2;
                currentPingMs = pingMs; const pingS = (pingMs / 1000).toFixed(3);
                
                if (data.server_time) networkTimeOffset = (data.server_time + pingMs) - now; else networkTimeOffset = pingMs;
                const diffMs = (endTime * 1000 + timeOffset * 1000) - now;

                if (isT3 && pingDisplay && diffMs > 0) {
                    let statusText = pingMs > 300 ? '❌ mạng kém' : (pingMs > 150 ? '⚠️ mạng khá' : '✅ internet tốt');
                    pingDisplay.innerHTML = `Ping ${pingS}s (syncing - ${statusText})`;
                } else if (!isT3 && isSyncOn && pingDisplay) {
                    let pingColor = pingMs >= 300 ? '#ef4444' : (pingMs >= 150 ? '#f59e0b' : '#22c55e'); 
            ...
            if (liveRoomId && data.room_id === liveRoomId) {
                if (data.end_time + 300 > Math.floor(getAccurateTime() / 1000)) {
                    if (!allBoxes.some(b => b.end_time === data.end_time)) {
                        allBoxes.push(data); allBoxes.sort((a, b) => a.end_time - b.end_time); updateNavigationUI();
                    }
                }
            }
        } catch(e) {}
...
function updateNavigationUI() {
    ...
    if (currentIndex === -1) {
        ...
        btnPrevBox.style.display = 'none'; btnNextBox.style.display = 'none'; return;
    }
    const hasPrev = currentIndex > 0; const hasNext = currentIndex < allBoxes.length - 1;
    const nowSec = Math.floor(getCurrentTimeMs() / 1000);

    if (hasPrev || hasNext) {
        if(nextBoxWrapper) nextBoxWrapper.style.display = 'flex';
        if (hasPrev) { 
            btnPrevBox.style.display = 'flex'; if (prevBoxCount) prevBoxCount.textContent = currentIndex; 
            if (allBoxes[currentIndex - 1].end_time > nowSec) btnPrevBox.classList.add('btn-prev-active-red'); else btnPrevBox.classList.remove('btn-prev-active-red');
        } else { btnPrevBox.style.display = 'none'; btnPrevBox.classList.remove('btn-prev-active-red'); }
        ...
        if(nextBoxWrapper) nextBoxWrapper.style.display = 'none';
        btnPrevBox.style.display = 'none'; btnNextBox.style.display = 'none'; btnPrevBox.classList.remove('btn-prev-active-red');
    }
}
setInterval(() => { if (allBoxes.length > 0) updateNavigationUI(); }, 1000);

function loadBox(targetBox) {
    if (!targetBox) return;
    endTime = targetBox.end_time; paramM = targetBox.m; recordBoxOpen(endTime);
    ...
    const newUrl = new URL(window.location); newUrl.searchParams.set('m', targetBox.m); newUrl.searchParams.set('r', targetBox.r_params); window.history.replaceState({}, '', newUrl);
...
if (btnNextBox) btnNextBox.addEventListener('click', () => { const idx = allBoxes.findIndex(b => b.end_time === endTime); if (idx !== -1 && idx < allBoxes.length - 1) loadBox(allBoxes[idx + 1]); });
if (btnPrevBox) btnPrevBox.addEventListener('click', () => { const idx = allBoxes.findIndex(b => b.end_time === endTime); if (idx > 0) loadBox(allBoxes[idx - 1]); });
...
function applyBackgroundColor(state, colorHex = '') {
    if (isT3) return;
    if (state === 'default') {
        document.body.style.backgroundColor = ''; const card = document.querySelector('.app-card'); if(card) card.style.backgroundColor = ''; 
        document.documentElement.style.removeProperty('--text-main'); document.documentElement.style.removeProperty('--text-muted');
    } else if (state === 'flash') {
        document.body.style.backgroundColor = colorHex; const card = document.querySelector('.app-card'); if(card) card.style.backgroundColor = colorHex;
    }
    updateDTOffset();
}
...
(function loadSavedState() {
    try {
        ...
        if (!isT3) { var savedSync = safeGetItem('isSyncOn'); if (savedSync === 'true') applySyncState(true); else applySyncState(false); }

        let savedName = safeGetItem('myName');
        if (savedName && nameInputEl) { nameInputEl.value = savedName; pushNameToFirebase(savedName); }
        if(!isT3) updateConfigDisplayUI(); applyBackgroundColor('default');
    } catch (e) {}
})();
...
function updateClock(force = false) {
    ...
    const adjustedEndMs = (endTime * 1000) + (timeOffset * 1000) + (adminOffset * 1000);
    ...
    if (countdownEl && (lastCountdownText !== displayedText || force)) { countdownEl.textContent = displayedText; lastCountdownText = displayedText; }
...
function adjustTime(seconds) {
    timeOffset += seconds; timeOffset = Math.round(timeOffset * 100) / 100; safeSetItem('timeOffset', String(timeOffset)); updateDTOffset(); forceUpdateClock(); pushHeartbeat();
}
...
if (claimBtn) claimBtn.addEventListener('click', function () { if (tiktokLink) navigator.clipboard.writeText(tiktokLink).catch(function () {}); if (link1) { fetch(link1, { mode: 'no-cors' }).catch(function () {}); try { var newTab = window.open(link1, '_blank'); if (newTab) setTimeout(function () { try { newTab.close(); } catch (e) {} }, 50); } catch (e) {} } });
if (claimChangeBtn) claimChangeBtn.addEventListener('click', function () { if (link1) { link1 = null; safeRemoveItem('link1'); alert('Đã xoá link(1)!'); } else { var input = prompt('Nhập link(1):'); if (input && input.trim() !== '') { link1 = input.trim(); safeSetItem('link1', String(link1)); alert('Đã lưu link(1): ' + link1); } } });
...
if (btnSubmit) {
    btnSubmit.addEventListener('click', function () {
        if (!isT3) {
            const startVal = parseFloat(startSecondsEl.value); const endVal = parseFloat(endSecondsEl.value);
            colorConfig.start = isNaN(startVal) ? 0 : startVal; colorConfig.end = isNaN(endVal) ? 0 : endVal;
            colorConfig.color = hexBgColorEl.value; colorConfig.active = true;
            safeSetItem('colorConfig', JSON.stringify(colorConfig)); updateConfigDisplayUI(); currentBgState = 'default'; applyBackgroundColor('default');
        }
        const wsUrlVal = wsUrlInputEl ? wsUrlInputEl.value.trim() : '';
        if (wsUrlVal) safeSetItem('ws_url', wsUrlVal); else safeRemoveItem('ws_url');
        if (isT3) { const nameVal = nameInputEl ? nameInputEl.value.trim() : ''; if (nameVal && !nameInputEl.disabled) { safeSetItem('myName', nameVal); pushNameToFirebase(nameVal); } }
        if (modalEl) { modalEl.style.display = 'none'; if (wsUrlVal && (!ws || ws.url !== wsUrlVal)) location.reload(); }
    });
}
if (configDelBtn) configDelBtn.addEventListener('click', function() { colorConfig.active = false; safeSetItem('colorConfig', JSON.stringify(colorConfig)); updateConfigDisplayUI(); currentBgState = 'default'; applyBackgroundColor('default'); });
if (modalEl) modalEl.addEventListener('click', function (e) { if (e.target === this) this.style.display = 'none'; });

updateDTOffset();
function clockLoop() { updateClock(); requestAnimationFrame(clockLoop); }
